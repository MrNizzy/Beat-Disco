...
describe('generateBpmOptions', () => {
  it('generates 5 options for a normal BPM', () => {
    ...
    expect(options[0].bpm).toBe(32)     // 0.25x
    expect(options[1].bpm).toBe(64)     // 0.5x
    expect(options[2].bpm).toBe(128)    // 1x
    expect(options[3].bpm).toBe(256)    // 2x
    expect(options[4].bpm).toBe(512)    // 4x
  })

  it('marks original and recommended options', () => {
    const options = generateBpmOptions(128)
    expect(options[2].isOriginal).toBe(true)
    expect(options[2].recommended).toBe(true) // 128 está en 120-220
    expect(options[3].recommended).toBe(false) // 256 fuera de rango
  })
...
describe('getRecommendedOption', () => {
  it('returns the one in 120-220 range', () => {
    ...
    expect(rec.factor).toBe(2) // 80*2 = 160
  })
